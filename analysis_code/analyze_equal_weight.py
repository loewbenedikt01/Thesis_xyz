...
output_dir_metrics = DATA_PATH / "results" / "metrics" / "equal_weight"
output_dir_plots = DATA_PATH / "results" / "plots" / "equal_weight"
output_dir_metrics.mkdir(parents=True, exist_ok=True)
output_dir_plots.mkdir(parents=True, exist_ok=True)
DATA_DIR = DATA_PATH / "results" / "data" / "equal_weight"
...
def generate_dynamic_benchmark_report(portfolio_df, title="Portfolio Performance Analysis", threshold=0.05) -> str:
    ...
    for _cname, _, _c_start, _c_trough, _c_end in CRISIS_PERIODS:
        # Snap each date to the nearest available index date
        _idx = df.index
...
def generate_crisis_comparison_charts(portfolio_df, benchmark_df, title_prefix="") -> str:
    ...
    SUBTEXT = "#64748B"
    RED     = "#DC2626"
    PORT_C  = "#1D6FA4"   # blue  — portfolio
    BENCH_C = "#F97316"   # orange — benchmark

    # Build benchmark cumulative price series (rebased later per crisis)
    ...
    def _safe_sharpe(lr):
        ...
        if vol == 0 or len(lr) < 2:
            return np.nan
        return annualized_return(lr) / vol

    def _safe_sortino(lr):
        ann_ret = annualized_return(lr)
        ds = np.minimum(lr, 0) ** 2
        dv = np.sqrt(ds.mean()) * np.sqrt(TRADING_DAYS_PER_YEAR)
        return ann_ret / dv if dv != 0 else np.nan
    ...
    for cname, _, c_start, _, c_end in CRISIS_PERIODS:
        ...
        # Normalized cumulative return: start = 1.0
        ...
        def _metrics(lr):
            ...
            return {
                ...
                'Sharpe Ratio'          : f"{_safe_sharpe(lr):.3f}",
        ...
        # Color cell values: green if portfolio better, red if worse
        def _cell_color(name, pval_str, bval_str):
            """Return (port_color, bench_color)."""
            try:
                pv = float(pval_str.replace('%', ''))
                bv = float(bval_str.replace('%', ''))
            except Exception:
                return TEXT, TEXT
            # Volatility: lower magnitude = better → treat as lower-is-better (positive number)
            # Max Drawdown, VaR, CVaR: all negative, closer to 0 = less bad → higher raw value = better
            higher_is_better = name not in ('Annualized Volatility',)
            if pv > bv:
                return ('#166534' if higher_is_better else RED, TEXT)
            elif pv < bv:
                return (RED if higher_is_better else '#166534', TEXT)
            return TEXT, TEXT
...
# ------
# Load benchmark daily log returns once
...
for freq_name in FREQUENCIES:
    ...
    df_port = pd.read_csv(csv_path, index_col='date', parse_dates=True)
    ...
    section_title = f'<h2 style="font-family:Inter,Arial,sans-serif;color:#1E293B;margin:40px 0 8px 70px">Equal Weight  ·  {freq_name} Rebalancing</h2>'
    div = generate_dynamic_benchmark_report(
        df_port,
        title=f"Equal Weight Portfolio  ·  {freq_name} Rebalancing",
        threshold=0.05,
    )
...
# Write single combined HTML — Plotly CDN loaded once in <head>
combined_report_path = output_dir_plots / "report_all_frequencies.html"
combined_html = """<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Equal Weight Portfolio — All Frequencies</title>
</head>
...
print(f"Combined report exported to: {combined_report_path}")