...
DATA_DIR = DATA_PATH / "results" / "data" / "lstm"
...
def load_and_average_runs(freq_name):
    """Discover all portfolio_lstm_{freq}_{run}.csv files, average log_return
    across runs on common dates, recompute cumulative_value. Returns (df, n_runs)."""
    dfs, run = [], 1
    while True:
        p = DATA_DIR / f"portfolio_lstm_{freq_name}_{run}.csv"
        if not p.exists():
            break
...
def load_and_average_stats(freq_name):
    """Discover all portfolio_lstm_{freq}_{run}_statistics.csv, average per
    ...
    while True:
        p = DATA_DIR / f"portfolio_lstm_{freq_name}_{run}_statistics.csv"
        if not p.exists():
            break
...
def generate_rf_statistics_report(stats_df, freq_name) -> str:
    ...
    fig_ts.update_layout(
        title=dict(text=f"LSTM Prediction Quality Over Time  ·  {freq_name}",
...
for freq_name in FREQUENCIES:
    ...
    section_title = (
        ...
        f'LSTM  ·  {freq_name} Rebalancing  <small style="font-size:0.6em;color:#64748B">({run_label})</small></h2>'
    )
...
<head>
  <meta charset="utf-8">
  <title>LSTM Portfolio — All Frequencies</title>
</head>
