...
output_dir_metrics = DATA_PATH / "results" / "metrics" / "hrp"
output_dir_plots = DATA_PATH / "results" / "plots" / "hrp"
output_dir_metrics.mkdir(parents=True, exist_ok=True)
output_dir_plots.mkdir(parents=True, exist_ok=True)
DATA_DIR = DATA_PATH / "results" / "data" / "hrp"
...
for freq_name in FREQUENCIES:
    ...
    section_title = f'<h2 style="font-family:Inter,Arial,sans-serif;color:#1E293B;margin:40px 0 8px 70px">HRP  ·  {freq_name} Rebalancing</h2>'
    div = generate_dynamic_benchmark_report(
        df_port,
        title=f"HRP Portfolio  ·  {freq_name} Rebalancing",
        threshold=0.05,
    )
...
<head>
  <meta charset="utf-8">
  <title>HRP Portfolio — All Frequencies</title>
</head>
