...
def load_and_average_runs(freq_name):
    """Discover all portfolio_xgb_{freq}_{run}.csv files, average log_return
    across runs on common dates, recompute cumulative_value. Returns (df, n_runs)."""
    dfs, run = [], 1
    while True:
        p = DATA_DIR / f"portfolio_xgb_{freq_name}_{run}.csv"
        if not p.exists():
            break
...
def load_and_average_stats(freq_name):
    """Discover all portfolio_xgb_{freq}_{run}_statistics.csv, average per
    rebalance_date. Returns averaged DataFrame or None."""
    dfs, run = [], 1
    STAT_COLS = ['best_learning_rate', 'best_max_depth', 'best_reg_lambda',
                 'best_n_estimators', 'best_gamma',
                 'RMSE', 'MSE', 'MAE', 'R_squared', 'Spearman',
                 'Directional_Accuracy', 'Geometric_Score']
    while True:
        p = DATA_DIR / f"portfolio_xgb_{freq_name}_{run}_statistics.csv"
        if not p.exists():
            break
...
def generate_rf_statistics_report(stats_df, freq_name) -> str:
    ...
    fig_ts.update_layout(
        title=dict(text=f"XGBoost Prediction Quality Over Time  ·  {freq_name}",
...
for freq_name in FREQUENCIES:
    ...
    section_title = (
        ...
        f'XGBoost  ·  {freq_name} Rebalancing  <small style="font-size:0.6em;color:#64748B">({run_label})</small></h2>'
    )

    # Main performance report
    div = generate_dynamic_benchmark_report(
        df_port,
        title=f"XGBoost  ·  {freq_name} Rebalancing  ({run_label})",
        threshold=0.05,
    )
