...
def build_freq_csv(freq_label, out_name):
    ...
    for key, data_subdir, file_tpl in MODELS:
        series = load_cum_return(data_subdir, file_tpl, freq_label)
        if series is None:
            print(f"  [{key}] file not found — skipping")
            continue
        series = series.rename(columns={"cum_return": f"cum_return_{key}"})
    ...
    out_path = os.path.join(OUT_DIR, f"cum_returns_pct_{out_name}.csv")
    combined.to_csv(out_path, index=False, float_format="%.6f")
    print(f"Saved cum_returns_pct_{out_name}.csv  ({len(combined)} rows, {len(combined.columns)} cols)")
