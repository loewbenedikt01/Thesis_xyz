...
# --- CONFIGURATION ---
TRAIN_MONTHS     = 60       # Training window
VAL_MONTHS       = 12       # Validation window (holdout — not seen by GridSearchCV)
MIN_COMPLETENESS = 0.50     # Min data history required per stock
WEIGHT_MAX       = 0.10     # Max 10% per stock
WEIGHT_MIN       = 0.01     # Min 1% per stock
N_RUNS           = 10       # Independent runs (different seeds)
RANDOM_SEED      = 41       # Base seed — each run uses RANDOM_SEED + run
CV_FOLDS         = 3        # Cross-validation folds inside GridSearchCV
...
for run in range(1, N_RUNS + 1):
    ...
    for label, (offset, horizon) in FREQUENCIES.items():
        print(f"  Processing XGBoost ({label}) | horizon={horizon}d | min coverage: {MIN_COMPLETENESS:.0%}")
            ...
            if target_year in universe.tickers:
                ...
                if not hist_prices.empty:
                    ...
                    if len(valid_tickers) >= 2:
                        ...
                        # Validation samples (holdout — only used for diagnostics after grid search)
                        ...
                        if X_train:
                            ...
                            gs = GridSearchCV(
                                ...
                                refit=False,   # we handle final fit ourselves
                                n_jobs=-1,
                            )
                            gs.fit(X_tr, y_tr)
                            best_params = gs.best_params_

                            # ── Evaluate best config on holdout val set ──────────
                            val_r2, val_da = np.nan, np.nan
                            if X_val:
                                X_vl = pd.concat(X_val)
                                y_vl = pd.concat(y_val)
                                ...
                                y_vl_pred = val_model.predict(X_vl)
                                ss_res = np.sum((y_vl.values - y_vl_pred) ** 2)
                                ss_tot = np.sum((y_vl.values - y_vl.values.mean()) ** 2)
                                val_r2 = 1 - ss_res / ss_tot if ss_tot != 0 else np.nan
                                val_da = np.mean((y_vl_pred >= 0.5) == (y_vl.values >= 0.5))

                            print(
                                f"  [{label}] {current_date.date()} — "
                                f"lr={best_params['learning_rate']} depth={best_params['max_depth']} "
                                f"λ={best_params['reg_lambda']} rounds={best_params['n_estimators']} "
                                f"γ={best_params['gamma']} | "
                                f"Val R²={val_r2:.3f}  Val DA={val_da:.1%}"
                            )

                            # ── Final model: retrain on train + val combined ─────
                            X_full = pd.concat(X_train + X_val) if X_val else X_tr
                            y_full = pd.concat(y_train + y_val) if X_val else y_tr
                            ...
                            # ── Predict at current rebalance date ────────────────
                            current_feat = create_features(train_prices)
                            if not current_feat.empty:
                                preds        = final_model.predict(current_feat)
        ...
        pd.DataFrame(portfolio_performance).to_csv(
            output_dir / f"portfolio_xgb_{label}_{run}.csv", index=False
        )
        pd.DataFrame(rebalance_details).to_csv(
            output_dir / f"portfolio_xgb_{label}_{run}_details.csv", index=False
        )
        pd.DataFrame(model_stats).to_csv(
            output_dir / f"portfolio_xgb_{label}_{run}_statistics.csv", index=False
        )
